...
--   Every password below is  JAILEAD
--
...
select v.roll, quiz.hash_password('JAILEAD'), v.name, b.id
from (values
...
values
  ('ADMIN1', quiz.hash_password('JAILEAD'), 'Admin 1'),
  ('ADMIN2', quiz.hash_password('JAILEAD'), 'Admin 2'),
  ('ADMIN3', quiz.hash_password('JAILEAD'), 'Admin 3'),
  ('ADMIN4', quiz.hash_password('JAILEAD'), 'Admin 4'),
  ('ADMIN5', quiz.hash_password('JAILEAD'), 'Admin 5')
...
union all
select 'STUDENT ' || s.roll_no || ' / JAILEAD', b.name, coalesce(s.full_name, '')
  from quiz.students s join quiz.batches b on b.id = s.batch_id where s.roll_no in ('30217','68459','15743','82096')
union all
select 'ADMIN   ' || username || ' / JAILEAD', '-', coalesce(display_name, '')
